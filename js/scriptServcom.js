var consulta_protocolo_automatica, consulta_certidao_automatica;
    ...
    !(function(a, b) {
        ...
        m.extend({
                clone: function(a, b, c) {
                    ...
                    if (
                        (k.html5Clone || m.isXMLDoc(a) || !gb.test("<" + a.nodeName + ">") ? (f = a.cloneNode(!0)) : ((tb.innerHTML = a.outerHTML), tb.removeChild((f = tb.firstChild))), !((k.noCloneEvent && k.noCloneChecked) || (1 !== a.nodeType && 11 !== a.nodeType) || m.isXMLDoc(a)))
                    )
                    ...
                    if (
                        ((o.promise(v).complete = p.add),
                            ...
                            (k.dataTypes = m
                                ...
                                .match(E) || [""]),
                            null == k.crossDomain &&
                            ((c = Gc.exec(k.url.toLowerCase())), (k.crossDomain = !(!c || (c[1] === yc[1] && c[2] === yc[2] && (c[3] || ("http:" === c[1] ? "80" : "443")) === (yc[3] || ("http:" === yc[1] ? "80" : "443")))))),
                    ...
                    if (f)
                        return (
                            ...
                            (typeof e.getBoundingClientRect !== K && (d = e.getBoundingClientRect()),
                                (c = dd(f)), { top: d.top + (c.pageYOffset || b.scrollTop) - (b.clientTop || 0), left: d.left + (c.pageXOffset || b.scrollLeft) - (b.clientLeft || 0) }) :
                            d
                        );
                },
                position: function() {
                    if (this[0]) {
                        ...
                        return (
                            ...
                            ((a = this.offsetParent()), (b = this.offset()), m.nodeName(a[0], "html") || (c = a.offset()), (c.top += m.css(a[0], "borderTopWidth", !0)), (c.left += m.css(a[0], "borderLeftWidth", !0))), { top: b.top - c.top - m.css(d, "marginTop", !0), left: b.left - c.left - m.css(d, "marginLeft", !0) }
                        );
                    }
                },
...
$(document).ready(function() {
    var hd = $("head");
    var bd = $("body");
    hd.append('<link href="./css/consultaServcom.css" rel="stylesheet">');
    bd.append(
        '<div id="CONSULTA_REGISTRO" class="servcomJanelaConsultaAndamento" style="display: none" data-opcao="CONSULTA_REGISTRO">' +
        '    <div class="janelaConsultaAndamentoContainer">' +
        '        <a href="javascript:void(0)" title="Fechar a janela" class="servcomJanelaConsultaAndamentoClose">X</a>' +
        "        <h2>>> Consulta Andamento Registro <small></small></h2>" +
        "        <p>" +
        "           <span>Informe o n&uacute;mero do Processo</span>" +
        '           <input id="numero" type="text" maxlength="15" class="input"/>' +
        '           <input id="consultar" type="button" class="botao" value="Consultar" />' +
        "        </p>" +
        "        <hr/>" +
        '        <div class="resultadoConsulta"></div>' +
        "    </div>" +
        "</div>"
    );
    bd.append(
        '<div id="CONSULTA_CERTIDAO" class="servcomJanelaConsultaAndamento" style="display: none" data-opcao="CONSULTA_CERTIDAO">' +
        '    <div class="janelaConsultaAndamentoContainer">' +
        '        <a href="javascript:void(0)" title="Fechar a janela" class="servcomJanelaConsultaAndamentoClose">X</a>' +
        "        <h2>>> Consulta Andamento Certid&atilde;o <small></small></h2>" +
        "        <p>" +
        "           <span>Informe o n&uacute;mero da Certid&atilde;o</span>" +
        '           <select id="ano" class="input" style="width: 70px">' +
    ...
    $(".servcomJanelaConsultaAndamento").click(function(obj) {
        var jan = $(obj.target);
        if (jan.hasClass("servcomJanelaConsultaAndamento")) {
            closeJanelaConsultaByObj(jan);
        }
    });
    $(".servcomJanelaConsultaAndamentoClose").click(closeJanelaConsulta);
    $(".servcomJanelaConsultaAndamento .botao").click(disparaConsultaAndamento);
    $(".servcomJanelaConsultaAndamento #numero").keypress(function(event) {
        if (event.which == 13) {
            disparaConsultaAndamento(event);
            event.preventDefault();
        } else if (event.which == 0) {
            closeJanelaConsulta(event);
        }
    });
    $(".consulta_andamento_registro").click(function() {
        JanelaConsultaRegistro();
    });
    $(".consulta_andamento_certidao").click(function() {
        JanelaConsultaCertidao();
    });
    AcionamentoAutomatico();
});

function GetUrlParam(name) {
    var results = new RegExp("[?&]" + name + "=([^&#]*)").exec(top.location.href);
    return (results != null && results.length) > 1 ? results[1] : null;
}

function AcionamentoAutomatico() {
    consulta_protocolo_automatica = GetUrlParam("cp");
    consulta_certidao_automatica = GetUrlParam("cc");
    if (consulta_protocolo_automatica != null) {
        JanelaConsultaRegistro();
    } else if (consulta_certidao_automatica != null) {
        JanelaConsultaCertidao();
    }
}

function JanelaConsultaRegistro() {
    showWindowConsultaAndamento($("#CONSULTA_REGISTRO"));
}

function JanelaConsultaCertidao() {
    showWindowConsultaAndamento($("#CONSULTA_CERTIDAO"));
}

function showWindowConsultaAndamento(jan) {
    jan.addClass("servcomJanelaConsultaAndamentoShow");
    jan.css("display", "inherit");
    setTimeout(function() {
        var numero = jan.find("#numero");
        numero.focus();
        if (consulta_protocolo_automatica != null) {
            numero.val(consulta_protocolo_automatica);
            var obj = { target: numero };
            disparaConsultaAndamento(obj);
        } else if (consulta_certidao_automatica != null) {
            numero.val(consulta_certidao_automatica);
            var obj = { target: numero };
            disparaConsultaAndamento(obj);
        }
    }, 200);
}
var janelaConsultaAndamento;

function disparaConsultaAndamento(obj) {
    janelaConsultaAndamento = $(obj.target).closest(".servcomJanelaConsultaAndamento");
    var op = janelaConsultaAndamento.data("opcao");
    var val = janelaConsultaAndamento.find("#numero").val();
    var vars = { OPER: op, NUMERO: val, ID_CLIENTE: "702", WEB: true };
    if (op == "CONSULTA_CERTIDAO") {
        var certidao_ano = GetUrlParam("cc_ano");
        if (certidao_ano != null) janelaConsultaAndamento.find("#ano").val(certidao_ano);
        var ano = janelaConsultaAndamento.find("#ano").val();
        vars.NUMERO = ano + "/" + val;
    }
    if (val.length == 0) {
        alert("Informe o valor que deseja consultar");
        return;
    } else if (isNaN(val)) {
        alert("Informe apenas valores numericos");
        janelaConsultaAndamento.find("#numero").val("");
        janelaConsultaAndamento.find("#numero").focus();
        return;
    }
    executaConsultaAndamentoServcom(vars, "consultaAndamentoResposta");
    enableDisableJanelaConsulta(janelaConsultaAndamento, false);
    janelaConsultaAndamento.find(".resultadoConsulta").html("");
}

function executaConsultaAndamentoServcom(vars, funcaoProcessaResposta) {
    var funcResposta = window[funcaoProcessaResposta];
    var isIe = detectIE();
    if (isIe) {
        if (isIe >= 10) {
            ...
            xhr.onerror = function(evt) {
                funcResposta({ MENSAGEM: "Requisição não autorizada para este domínio" });
            };
            xhr.open("post", "http://atualizacaoandamentos.servcom.com.br/Main/");
            ...
            try {
                xdr.open("post", "http://atualizacaoandamentos.servcom.com.br/Main/", true);
        ...
        vars.JSON_CALLBACK = funcaoProcessaResposta;
        $.ajax({ url: "http://atualizacaoandamentos.servcom.com.br/Main/", data: vars, type: "POST", dataType: "jsonp", crossDomain: true, contentType: "application/json" });
    }
}

function consultaAndamentoResposta(ret) {
    enableDisableJanelaConsulta(janelaConsultaAndamento, true);
    janelaConsultaAndamento.find("#numero").select();
    if (ret.RESULTADO) {
        var res = ret.RESULTADO;
        res = !$.isArray(res) ? [res] : res;
        var cont = "";
        $.each(res, function(i, v) {
            cont += v;
        });
        janelaConsultaAndamento.find(".resultadoConsulta").html(cont);
    } else {
        if (ret.MENSAGEM) {
            alert("ERRO: " + ret.MENSAGEM);
        } else if (ret.ERRO) {
            alert("ERRO: " + ret.ERRO);
        } else {
            alert("ERRO NAO IDENTIFICADO: Contacte o responsavel pelo sistema");
        }
        janelaConsultaAndamento.find("#numero").val("");
    }
}

function closeJanelaConsulta(obj) {
    var jan = $(obj.target).closest(".servcomJanelaConsultaAndamento");
    closeJanelaConsultaByObj(jan);
}

function closeJanelaConsultaByObj(jan) {
    if (jan.hasClass("servcomJanelaConsultaAndamentoShow")) {
        jan.removeClass("servcomJanelaConsultaAndamentoShow");
    }
}

function enableDisableJanelaConsulta(jan, modo) {
    if (!modo) {
        jan.find(".janelaConsultaAndamentoContainer").append('<div class="janelaProcessando">' + "   <div>" + "       <span>Processando. Aguarde!</span>" + "   </div>" + "</div>");
        jan.find("input").attr("disabled", "disabled");
    } else {
        jan.find(".janelaProcessando").remove();
        jan.find("input").removeAttr("disabled");
    }
}

function destroiAll() {
    $("#CONSULTA_REGISTRO").remove();
    $("#CONSULTA_CERTIDAO").remove();
}
